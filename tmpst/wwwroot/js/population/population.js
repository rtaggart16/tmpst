...
function createD3Visualisation(data, type) {
...
function radarChartBuilderMobile(data) {
...
function radarChartBuilder(data) {
    ...
    try {
        ...
        var allAxis = (dataInArray[0].map(function (i, j) { return i.axis })),	//Names of each axis
            total = allAxis.length,					//The number of different axes
            radius = container.height() / 2, 	//Radius of the outermost circle
            Format = d3.format(',d'),			 	//Percentage formatting
            angleSlice = Math.PI * 2 / total;		//The width in radians of each "slice"
    ...
    function wrap(text, width) {
        text.each(function () {
            var text = d3.select(this),
                ...
                x = text.attr("x"),
                dy = parseFloat(text.attr("dy")),
                tspan = text.text(null).append("tspan").attr("x", x).attr("y", y).attr("dy", dy + "em").attr("class", "chart-text");
...
function submitPopulationRequest() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            createD3Visualisation(result, 'noice');
